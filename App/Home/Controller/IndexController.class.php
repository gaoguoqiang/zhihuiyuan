...
class IndexController extends HomeController {
    public function index(){
		//浏览数量
		if(empty($_COOKIE['PHPSESSID'])){ //判断COOKIE的是否存在
			setcookie("PHPSESSID",md5(time())); //如果不存在,则创建COOKIE
			$number = M('number')->find();
			$count = $number['number'] + 1; //将变量$count的值加1
			$data['number'] = $count;
			$re = M('number')->where('id = 0')->save($data);
			//储存访问用户的信息
			$renm['userip'] = $_SERVER['REMOTE_ADDR'];
			$r = M('userip')->where($renm)->find();
    ...
    public function commitList1(){
    	if($_GET['nid'] == 16){
			...
			header("Location: ".__ROOT__."/Train/index/nid/23");
		}elseif(52<$_GET['id'] &&$_GET['id']<57){
			header("Location: ".__ROOT__."/Train/index/nid/26/id/".$_GET['id']);
		}elseif($_GET['nid'] == 26 && $_GET['id']){
			header("Location: ".__ROOT__."/Train/index/nid/26/id/".$_GET['id']);
		}elseif(57<$_GET['id'] &&$_GET['id']<62){
			header("Location: ".__ROOT__."/Train/index/nid/26/id/".$_GET['id']);
		}elseif($_GET['nid'] == 27 && $_GET['id']){
			header("Location: ".__ROOT__."/Train/index/nid/27/id/".$_GET['id']);
    	}else{
		    	if(count($_GET)==1){
		    	//面包屑
			    	if($_GET['nid']){
