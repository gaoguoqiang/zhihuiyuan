...
class ReteacherController extends HomeController {
	public function index(){
		...
		//讲师显示分页效果
		    ...
		    $result = $db->table('si_users n, si_teacher i')->where('n.tid = i.id')->field('n.id as id, n.user as user, n.sex as sex,n.email as email,n.phone as phone,i.addr as addr,i.field as field,i.technology as technology,i.introduction as introduction,i.company as company,i.face as face,i.prov as prov,i.city as city,i.dist as dist,i.teacher as teacher')->order('createtime desc')->page($page,6)->select();
		    $count = $db->count();
		    for ($i=0; $i <= $count; $i++) { 
